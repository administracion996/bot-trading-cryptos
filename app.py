...
st.set_page_config(
    page_title="Crypto Trading Bots Dashboard", page_icon="🛡️", layout="wide"
)
...
tab_sniper, tab_cazador = st.tabs(["🎯 Bot 1: Francotirador", "⚡ Bot 2: Cazador"])

# ==========================================
# PESTAÑA 1: FRANCOTIRADOR (SNIPER)
# ==========================================
with tab_sniper:
...
# ==========================================
# PESTAÑA 2: CAZADOR (CAZADOR DE MOMENTUM)
# ==========================================
with tab_cazador:
    ...
    if cartera_c:
        ...
        capital_total_c = round(efectivo_c + capital_invertido_c, 2)

        # 1. MÉTRICAS GLOBALES CAZADOR
        col1_c, col2_c, col3_c, col4_c = st.columns(4)
        col1_c.metric("Capital Activo (Trabajo)", f"{capital_total_c:.2f} €")
        col2_c.metric("Efectivo Libre", f"{efectivo_c:.2f} €")
        col3_c.metric("🏦 Reserva Intocable", f"{vault_c:.2f} €")
        col4_c.metric("📌 Posiciones Abiertas", len(posiciones_c))

        st.divider()

        # 2. TABLA DE POSICIONES DETALLADA CAZADOR
        st.subheader("📌 Posiciones Actuales en Cartera (Cazador)")
        if posiciones_c:
            filas_pos_c = []
            for ticker, pos in posiciones_c.items():
                unidades = float(pos.get("unidades", 0.0))
                precio_compra = float(pos.get("precio_compra", 0.0))
                coste_total = float(pos.get("coste_total", 0.0))
                max_precio = float(pos.get("max_precio_alcanzado", precio_compra))
                confianza = pos.get("confianza_gemini", "N/A")
                fecha_ent = pos.get("fecha_entrada", "")[:19].replace("T", " ")

                filas_pos_c.append({
                    "Activo": ticker.replace("-USD", ""),
                    "Unidades": unidades,
                    "Precio Compra ($)": round(precio_compra, 8),
                    "Invertido (€)": round(coste_total, 2),
                    "Max Alcanzado ($)": round(max_precio, 8),
                    "Confianza Gemini": f"{confianza}/100" if confianza != "N/A" else "N/A",
                    "Fecha Entrada": fecha_ent
                })

            st.dataframe(pd.DataFrame(filas_pos_c), use_container_width=True)
        else:
            st.info("Sin posiciones abiertas en este momento. El bot está escaneando el mercado.")

        st.divider()

        # 3. HISTORIAL DE OPERACIONES CAZADOR
        st.subheader("📜 Historial de Operaciones Cerradas (Cazador)")
        if historial_c:
            df_hist_c = pd.DataFrame(historial_c)
            
            pnl_total_c = df_hist_c["beneficio_neto"].sum() if "beneficio_neto" in df_hist_c else 0.0
            ventas_cerradas_c = len(df_hist_c)

            col_m1_c, col_m2_c = st.columns(2)
            col_m1_c.metric("💰 PnL Realizado Histórico", f"{pnl_total_c:+.2f} €")
            col_m2_c.metric("🔄 Ventas Ejecutadas", ventas_cerradas_c)

            st.write("")

            df_hist_view_c = pd.DataFrame({
                "Fecha Salida": df_hist_c.get("fecha_salida", "").str[:19].str.replace("T", " "),
                "Activo": df_hist_c.get("ticker", "").str.replace("-USD", ""),
                "Resultado PnL (%)": df_hist_c.get("pnl_pct", 0.0).apply(lambda x: f"{'+' if x > 0 else ''}{x:.2f}%"),
                "Beneficio Neto (€)": df_hist_c.get("beneficio_neto", 0.0).apply(lambda x: f"€{x:.4f}"),
                "Precio Entrada ($)": df_hist_c.get("precio_entrada", 0.0).apply(lambda x: f"${x:.8f}"),
                "Precio Salida ($)": df_hist_c.get("precio_salida", 0.0).apply(lambda x: f"${x:.8f}"),
                "Motivo Salida": df_hist_c.get("motivo_salida", "")
            })

            st.dataframe(df_hist_view_c.iloc[::-1], use_container_width=True, height=400)
        else:
            st.info("Aún no hay operaciones cerradas en el historial del Cazador.")
    else:
