...
if cartera:
  ...
  if activos_seleccionados:
    ...
    for ticker in activos_seleccionados:
      try:
        df_hist = yf.Ticker(ticker).history(period="5d", interval="15m")
          ...
          for op in historial_crudo:
            ...
            try:
              ...
              if "COMPRA" in op or "INICIO" in op or "DCA" in op:
                ...
                label_compra = (
                    "🟢 DCA SALVAVIDAS" if "SALVAVIDAS" in op else "🟢 COMPRA"
                )
                textos_compra.append(
                    f"{label_compra} {ticker}<br>Hora:"
