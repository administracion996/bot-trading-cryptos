...
if cartera:
  ...
  # 2. GRÁFICA INICIAL DE BARRAS (DISTRIBUCIÓN ORIGINAL)
  ...
  for ticker, pos in posiciones.items():
    valor_est = round(
        float(pos.get("cantidad", 0)) * float(pos.get("precio_entrada", 0)), 2
    )
    datos_barras["Activo"].append(ticker.replace("-USD", ""))
    datos_barras["Valor (€)"].append(valor_est)
  ...
  # 5. GRÁFICA LINEAL AL FINAL (OPCIONAL Y AISLADA DE ERRORES)
  ...
  if seleccionadas:
    try:
      ...
      st.caption("Gráfica de tendencia no disponible temporalmente.")
